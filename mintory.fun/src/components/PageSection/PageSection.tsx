...
const PageSection: React.FC<PageSectionProps> = ({ bgColor, className, getData, title, }) => {
    ...
    return (
        <div className={`${bgColor ? `bg-[${bgColor}]` : 'white'} ${className} px-20 py-8`}>
            <div className='flex justify-between pb-4 text-[#7452EE]'>
                <p className='text-5xl font-bold '>{title}</p>
                <button className='bg-transparent'>see all</button>
            </div>
            <div className='flex flex-wrap w-full items-center justify-center md:justify-normal gap-6 py-6'>
            {data.map((nft, index) => {
                return (
